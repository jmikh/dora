"""
Generate complaints data JSON for the demo site dashboard.
...
# Output path
OUTPUT_FILE = Path(__file__).parent / "data" / "complaints.json"
...
def generate_ai_summary(category: str, complaints: list[dict]) -> str:
    """
    ...
    try:
        ...
        complaint_texts = [f"- {c['complaint']}: \"{c['quote']}\"" for c in complaints[:15]]
        context = "\n".join(complaint_texts)

        prompt = f"""Summarize these user complaints about Wispr Flow in the "{category}" category in 2-3 sentences. Focus on the key issues and patterns. Be concise and professional.
...
Summary:"""
        ...
        return f"This category contains {count} complaints from users experiencing {category.lower()}."


def main():
    """Main function to generate complaints JSON."""
    ...
    with Session(engine) as session:
        # Query all complaints with categories and source info
        print("Fetching complaints with source data...")
        ...
        query = text("""
            ...
            FROM complaints c
            LEFT JOIN complaint_categories cc ON c.id = cc.complaint_id
            LEFT JOIN reddit_content rc ON c.source_table = 'reddit_content' AND c.source_id = rc.id
            LEFT JOIN reviews r ON c.source_table = 'reviews' AND c.source_id = r.review_id
            ORDER BY cc.category, c.extracted_at DESC
        """)
        ...
        for row in results:
            ...
            if source_type == "reddit_content":
                ...
                complaint_entry["source"] = {
                    ...
                    "url": None  # Reviews don't have direct URLs
                }

            complaint_entry["icon"] = get_source_icon(source_type, source_name)
            cat_data["complaints"].append(complaint_entry)
        ...
        for cat_data in sorted_categories:
            print(f"  Processing: {cat_data['name']} ({cat_data['count']} complaints)")
        ...
        for cat in sorted_categories[:5]:
            print(f"  - {cat['name']}: {cat['count']} complaints")
